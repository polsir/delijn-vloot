...
if "map_center" not in st.session_state: st.session_state.map_center = [51.05, 4.33]
...
# --- DE OPLOSSING VOOR HET HERLADEN ---
# We gebruiken 'use_container_width' en beperken de returned objects
output = st_folium(
    m,
    width=1400,
    height=600,
    ...
    debug=False # Voorkomt extra logs/refreshes
)

# Sla positie alleen op als deze echt veranderd is door de GEBRUIKER
if output and output.get("center"):
    c = output["center"]
    # Afronden op 4 decimalen om 'micro-refreshes' door afrondingsfouten te voorkomen
    new_lat, new_lon = round(c["lat"], 4), round(c["lng"], 4)
    old_lat, old_lon = round(st.session_state.map_center[0], 4), round(st.session_state.map_center[1], 4)
