...
with st.sidebar:
    ...
    for b_id in st.session_state.fleet:
        ...
        col1.write(f"🚌 {b_id}")
        if col2.button("🗑️", key=f"del_{b_id}"):
            st.session_state.fleet.remove(b_id)
            st.rerun()
    
    st.divider()
    if st.button("🔄 Reset Alle Tellers"):
        for b_id in st.session_state.fleet:
...
def create_base_map():
    ...
    folium.TileLayer(tiles='https://mt1.google.com/vt/lyrs=m,traffic&x={x}&y={y}&z={z}', attr='Google Traffic', name='Verkeer', overlay=True, opacity=0.7).add_to(m)
    return m
...
with tab1:
    ...
    for b_id in st.session_state.fleet:
        ...
        st.markdown(f'<div class="stat-card" style="border-top-color:{"#2ecc71" if in_z else "#3498db"};"><b>Bus {b_id}</b><br><span style="font-size:24px; color:#2c3e50;">{st.session_state.counter[b_id]} ritten</span></div>', unsafe_allow_html=True)
